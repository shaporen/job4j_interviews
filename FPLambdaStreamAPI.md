...
4. Результат: возвращается новый список
...
Методы `allMatch()`, `noneMatch()` и `anyMatch()` в Stream API Java используются для проверки условий, которые применяются ко всем элементам потока.

**Метод `allMatch()`**
* Описание: проверяет, соответствует ли каждый элемент потока заданному предикату. 
* Возвращаемое значение: `true`, если все элементы соответствуют предикату, `false` - иначе.
...
**Метод `noneMatch()`**
* Описание: Проверяет, не соответствует ли ни один элемент потока заданному предикату.
* Возвращаемое значение: `true`, если ни один элемент не соответствует предикату, `false` - иначе.
  
Пример:
```java
List<Integer> numbers = Arrays.asList(1, 3, 5, 7);

boolean noneEven = numbers.stream().noneMatch(n -> n % 2 == 0); // true
```

**Метод `anyMatch()`**
* Описание: Проверяет, соответствует ли хотя бы один элемент потока заданному предикату.
* Возвращаемое значение: `true`, если хотя бы один элемент соответствует предикату, `false` - иначе.
...
Важно: 
* Эти методы работают с "ленивой" оценкой: они останавливают проход по потоку, как только результат проверки становится известен (например, в `allMatch()` если найдется элемент, не соответствующий предикату, проверка останавливается).
* `noneMatch()` фактически является обратной операцией к `anyMatch()`.

Подводя итог:
* `allMatch()` - проверка соответствия всех элементов.
* `noneMatch()` - проверка соответствия ни одного элемента.
* `anyMatch()` - проверка соответствия хотя бы одного элемента.
