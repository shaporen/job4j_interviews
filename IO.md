...
**Класс Scanner в Java** — это мощный инструмент для считывания входных данных из различных источников, таких как консоль, файлы, строки и даже сетевые потоки. Он предоставляет удобные методы для считывания данных в разных форматах, например:
+ Целые числа (int, long, short)
...
+ `next()`: считывает следующий токен из входного потока. Токен — это последовательность символов, разделенная пробелами, табуляциями или символами новой строки.
+ `nextInt()`: считывает целое число из входного потока.
+ `nextDouble()`: считывает число с плавающей точкой из входного потока.
+ `nextLine()`: считывает всю строку из входного потока, включая пробелы.
+ `hasNext()`: проверяет, есть ли следующий токен во входном потоке.
+ `hasNextInt()`: проверяет, является ли следующий токен целым числом.
+ `hasNextDouble()`: проверяет, является ли следующий токен числом с плавающей точкой.
+ `hasNextLine()`: проверяет, есть ли следующая строка во входном потоке.
+ `close()`: закрывает Scanner и освобождает связанные ресурсы.
...
public class ScannerExample {
    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in); // Создаем Scanner для консольного ввода

        System.out.print("Введите ваше имя: ");
        String name = scanner.nextLine(); // Считываем строку (имя)

        System.out.print("Введите ваш возраст: ");
        int age = scanner.nextInt(); // Считываем целое число (возраст)
...
+ Удобство: `Scanner` упрощает считывание данных из различных источников.
+ Гибкость:  `Scanner` позволяет считывать данные в разных форматах.
+ Проверка типов: `Scanner` автоматически проверяет, соответствует ли считываемое значение ожидаемому типу данных.

**Важно:**
+ Всегда закрывайте `Scanner` после использования с помощью метода `close()`, чтобы освободить ресурсы.
+ Обрабатывайте возможные исключения, которые могут возникнуть при работе с `Scanner`. Например, `InputMismatchException` может возникнуть, если пользователь вводит неверный тип данных.

**Дополнительные возможности:**
+ `Scanner` поддерживает работу с разделителями, которые определяют, как разделяются токены.
+ `Scanner` позволяет использовать различные кодировки для считывания данных.

В целом, класс `Scanner` является удобным инструментом для считывания входных данных в Java. Он упрощает разработку консольных приложений, парсинга данных из файлов и сетевых потоков.
