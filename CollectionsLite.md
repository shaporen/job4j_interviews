...
+ Порядок элементов определяется заданным компаратором.
...
1. `ArrayDeque`: использует массив для хранения элементов.
...
2. `LinkedList` - использует двусвязный список для хранения элементов.
...
3. `ConcurrentLinkedDeque` - использует двусвязный список для хранения элементов. Обеспечивает потокобезопасность.
