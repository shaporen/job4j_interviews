...
4. Процесс итерации:
   ...
   1) Цикл `foreach` вызывает метод `iterator()` на итерабельном объекте.
   2) Он получает объект `Iterator` и использует его для перебора элементов.
   3) Метод `hasNext()` используется для проверки, есть ли следующий элемент.
   4) Если есть, метод `next()` возвращает следующий элемент.
   5) Элемент присваивается переменной в цикле `foreach`.
   6) Тело цикла `foreach` выполняется.
   7) Процесс повторяется, пока `hasNext()` не вернет `false`.
