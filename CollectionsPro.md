...
+ ***CopyOnWriteArrayList*** - hеализация, обеспечивающая безопасный итератор при изменении списка. Итератор работает с копией списка на момент создания итератора.
+ ***ImmutableList*** - hеализация неизменяемого списка, доступная в сторонних библиотеках, таких как Guava. Не позволяет изменять содержимое списка после создания.
...
* Сдвиг вправо со знаком (`>>`): Перемещает биты числа вправо, заполняя освободившиеся позиции знаком старшего бита. Эквивалентно делению на степень двойки, равную числу сдвигов, с сохранением знака.
* Сдвиг вправо без знака (`>>>`): Перемещает биты числа вправо, заполняя освободившиеся позиции нулями. Эквивалентно делению на степень двойки, равную числу сдвигов, с принудительным положительным результатом.
